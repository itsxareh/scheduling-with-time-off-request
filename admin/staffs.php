	...
	input[type=checkbox]
{
  ...
  padding: 10px;
	...
	@media (max-width: 564px){
		...
		<div class="row">
			<div class="col-md-12 staff-table ps-0">
				<div class="card">
					<div class="card-header">
						<b>Staffs</b>
						<span class=""><button class="btn btn-primary btn-block btn-sm col-sm-2 float-right" type="button" id="new_staff">
							...
							<tbody>
								...
								<tr>
									...
									<td data-title= "Contact No." class="td">
										 <p><?php echo $row['contact'] ? $row['contact'] : "NONE"?></p>
									</td>
									...
									<td data-title="Action" class="text-center">
										<button class="btn btn-sm btn-outline-primary view_staff" type="button" data-id="<?php echo $row['id'] ?>" >View</button>
										<button class="btn btn-sm btn-outline-primary edit_staff" type="button" data-id="<?php echo $row['id'] ?>" >Edit</button>
										<button class="btn btn-sm btn-outline-primary delete_staff" type="button" data-id="<?php echo $row['id'] ?>">Delete</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('#print_table').click(function(){
        var nw = window.open("print_staffs.php","_blank","height=500,width=800")
        setTimeout(function(){
            nw.print()
            setTimeout(function(){
                nw.close()
