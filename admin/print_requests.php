...
<table>
    ...
    <center><h2>Time-off Requests</h2></center>
    <thead>
        <tr>
            <td>#</td>
    ...
    <tbody>
        ...
        <?php 
            $i = 1;
                $qry = $conn->query("SELECT *, CONCAT(e.lastname, ', ' , e.firstname,' ', COALESCE(e.middlename,'')) as name from staff e inner join `time-off-request` pp on pp.id_no = e.id_no order by from_date asc");
