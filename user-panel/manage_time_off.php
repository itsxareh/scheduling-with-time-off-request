...
<div class="container fluid">
        ...
        <div class="form-group">
            ...
            <select class="custom-select" name="leave_type" autocomplete="off">
                <option value="">Click here to select any ...</option>
                <?php
                $leavetype = $conn->query("SELECT * FROM leave_types");
                while($row= $leavetype->fetch_array()):
                ?>
                <option value="<?php echo ucwords($row['leave_type']) ?>"<?php if ($row['leave_type'] === $leave_type) echo 'selected'; ?>><?php echo ucwords($row['leave_type']) ?> - <?php echo ucwords($row['description']) ?></option>
            <?php endwhile; ?>
            </select>
        </div>
        <div class="form-group">
